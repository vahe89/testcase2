        ...
        if (isset($conf_menu) && is_array($conf_menu)) {
            foreach ($conf_menu as $ck => $cv) {
                ...
                <li <?php echo($ck == $conf_page_id ? "class='active'" : ''); ?>>
                    <a href = '<?php echo aurl($cv['_url']); ?>' ><?php echo $cv['_html']; ?></a ></li >
            <?php }
        } ?>
    </ul >
</div >

<form action = '<?php echo aurl("/"); ?>' method = 'POST' >
    <input type = 'hidden' name = '<?php echo $this->_secureFormNames('a'); ?>' value = '<?php echo $form_a; ?>' >
    <input type = 'hidden' name = '<?php echo $this->_secureFormNames('redirect_url'); ?>' value = '<?php echo aurl($redir_url); ?>' >
