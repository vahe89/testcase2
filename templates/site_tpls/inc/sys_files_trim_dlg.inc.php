<?php
if (!isset($_SESSION['class_user']['isAdmin']) || $_SESSION['class_user']['isAdmin'] !== true)
    die("Access denied");


$_REQUEST['file'] = $this->path . $_REQUEST['file'];
...
<div id = "#trim_plugin_content" >
    <script type = "text/javascript" >
        ...
        function toggleMask() {
            if ($('#msk').val() == 'Mask') {
                ...
                $('#sel').bind('resize', function (event, ui) {
                    setVals();
                    mask();
                });
                $('#sel').bind('resizestop', function (event, ui) {
                    setVals();
                    mask();
                });
                $('#msk').val('Unmask');
        ...
        /*
         ...
         $('#quality').slider({value:100,slide: function(event, ui) {$('#q').html(ui.value); }});
         $('#sel').draggable({ containment: "#container", scroll: false,drag:function(event,ui){setVals();},stop:function(event,ui){setVals();}});
         ...
         $('#sel').resizable({aspectRatio:15/10, containment: "#container",resize:function(event,ui){setVals();},stop:function(event,ui){setVals();} });
         setVals();
         });*/
    </script >
    <style >
    ...
    <form id = "trim_plugin_form" action = "index.php" method = "POST" onsubmit = "return trim_sub()" >
        ...
        <div style = "margin-bottom:5px;padding-bottom:3px;border:2px outset black;" >
            <table border = 0 cellpadding = 0 cellspacing = 0 width = 100% align = "middle" >
                <tr >
                    ...
                    <td align = 'center' >
                        Image quality:<span id = "q" >100</span >

                        <div id = "quality" style = "width:400px;" ></div >
                    </td >
                    <td align = 'center' width = "20%" >
        ...
        <center ><p >
                X:<span id = "dx" ></span > Y:<span id = "dy" ></span >
                Width:<span id = "dw" ></span >Height:<span id = "dh" ></span >
            </p ></center >


        <div id = "container" style = "margin:auto;position:relative;width:<?php echo $width ?>px;height:<?php echo $height ?>px;background:white;" >
