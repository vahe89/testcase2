...
"> | <input type="button" value="Trim" onclick="trim_plugin('<?php echo "$path/$d";?>',false,false,false,false,'#<?php echo $css_prefix?>slideshow img','#<?php echo $css_prefix?>thumbs .selected img')">
</div>
</li>
...
?>
<li>
<a class="thumb <?php echo $css_prefix?>gallery_empty" name="this-plugin-empty.gif" title="" href="./fckbrowser.plugin.php?draw_empty=1">
<img src="./fckbrowser.plugin.php?draw_empty=1" alt="" />
</a>
<div class="caption">
</div>
</li>
...
<script type="text/javascript" src="<?php echo $root_path?>js/ckeditor/ckeditor.js"></script>
...
{
	window.opener.CKEDITOR.tools.callFunction(<?php echo $_REQUEST['CKEditorFuncNum']?>,'<?php echo $path?>/'+fl);
	window.close();
}
		...
		onComplete : function(response, formId){
			...
			if( response == false ){
				...
				<?php echo $css_prefix?>gallery.insertImage('<li>																							\
					...
					"> |  <input type="button" value="Trim" onclick="trim_plugin(\'<?php echo $path?>/'+response.img+'\',false,false,false,false,false,\'#<?php echo $css_prefix?>slideshow img\',\'#<?php echo $css_prefix?>thumbs .selected img\')"> \
					</div> \
					</li>',0);
...
<div>
Add image:<form action='fckbrowser.plugin.php?ajax=1' method="POST" id="<?php echo $css_prefix?>ajax_gall" style="display:inline">
...
<form action='fckbrowser.plugin.php' method="POST" style="display:inline" enctype="multipart/form-data">
